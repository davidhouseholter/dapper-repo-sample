...
using Core.Repository.Attributes.LogicalDelete;
    ...
    where TEntity : class
{
    private string AppendJoinToUpdate<TBase>(TBase entity, SqlQuery originalBuilder, params Expression<Func<TEntity, object>>[] includes) where TBase : notnull
    {
        ...
        foreach (var include in includes)
        {
            ...
            var tableName = RepositoryOrmConfig.TablePrefix + (tableAttribute != null ? tableAttribute.Name : declaringType?.Name);
