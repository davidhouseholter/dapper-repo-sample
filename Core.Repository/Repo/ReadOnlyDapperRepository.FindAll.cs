    ...
    where TEntity : class
{
    ...
    public virtual IEnumerable<TEntity> FindAll(Expression<Func<TEntity, bool>>? predicate, Expression<Func<TEntity, object>>[] includes)
    {
        var queryResult = SqlGenerator.GetSelectAll(predicate, FilterData, includes);
        Console.WriteLine(queryResult.GetSql());
